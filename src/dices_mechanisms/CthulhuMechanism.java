 ...
 * must be below a threshold decided by the Game Master.
 * @author Nicolas Brax
 */
public class CthulhuMechanism implements GameMechanism {
   ...
   * launch can use different dice according to contexts. In Call of Cthulhu, 
   ...
   * die is launched and then the score is analysed to give the result. This 
   * result is computed as follows: 
   *  - Below the threshold is a success, above is a failure;
   *  - 1 is a critical success;
   *  - 100 or 96+ is a critical failure according to the threshold 
   *    (<50 or >50).
   * @param exp the expression to analyse, IT MUST BE 
   *            "1D100 --thresholdT --difficultyD".
  ...
  public String launchDices(String exp) throws RPG_DiceException{
    ...
    try{
       int threshold = Integer.parseInt(
                                    exp.split("--")[1].split("hold")[1].trim());
       int difficulty = Integer.parseInt(
                   exp.split("--difficulty")[1].replace("(static)", "").trim());
       int realThreshold = threshold / ((difficulty == 1) ? 1 :                 // modify the threshold according  
                                                  ((difficulty == 2) ? 2 : 10));// to the difficulty
      ...
      throw new RPG_DiceException("Can't analyse the dice expression for "
              + "                                        the Call of Cthulhu.");
    }
    return toReturn;
  }
  ...
  public String mechanismQuickDescription(){
    return "In Call of Cthulhu, besides launches for damage and other "
           + "computation, tests are made using a 100 faces die. The score is "
           + "then compared to a threshold modified according to a difficulty "
           + "level. There are three levels of difficulty --normal, difficult, "
           + "extreme-- that respectively modifies the value NOT to exceed "
           + "--threshold, threshold/2,threshold/10--. Moreover, critical "
           + "success occurs on a score of 1 and critical failure occurs on a "
           + "score of 100 if tthe hreshold is above 50 and on 96-100 if the "
           + "threshold is below 50.";
  }
}
