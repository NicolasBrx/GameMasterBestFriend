 ...
 * @author Nicolas Brax
 */
public class FengShuiMechanism implements GameMechanism {
   /**
   * The positive and negative launched dice.
   */
  private int positiveDice, negativeDice;
   ...
   * @param exp the expression to analyse, IT MUST BE "2D6".
  ...
  public String launchDices(String exp) throws RPG_DiceException{
    ...
    try{
      ...
      toReturn += "[" + this.positiveDice + "+, " + this.negativeDice 
               +"-] --result " + (this.positiveDice - this.negativeDice) ;
    }
    catch(Exception e){
  ...
  public String mechanismQuickDescription(){
    return "In Feng Shui, it launches two six faces dices, one positive and "
            + "one negative and add their values. The value of related "
            + "attribute or skill can be added using the --addX option where "
