...
import scenarios.ScenarioGraph;
...
import exceptions.RPG_ScenarioManagementException;
import tools.xmlloader.XML_ScenarioLoader;
 ...
 * @author Nicolas Brax
 */
public class IHM_ScenarioCreation extends javax.swing.JFrame {
  ...
  public IHM_ScenarioCreation() {
    ...
    XML_ScenarioLoader xml = new XML_ScenarioLoader();                                                // retrieving all the available scenarii
    ...
    scenarioGraph = new ScenarioGraph(this);                                    // graph viewer initialisation
  ...
  public void saveScenario(){
    XML_ScenarioLoader xml = new XML_ScenarioLoader();
  ...
  private void jbtnLoadScenarioActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnLoadScenarioActionPerformed
    // TODO: remove the current scenario if one is displayed in the graph view or it crashes since 
    //       it is unloaded in the core but not in the view
    XML_ScenarioLoader xml = new XML_ScenarioLoader();
    ...
    for(String id : this.myScenario.getWholeScenario().keySet()){               // for each element in the scenario
      if(this.myScenario.getElement(id).hasNext()){                             // if the node has next elements
        for(String next : myScenario.getElement(id).getNextElements()){         // for each next elements of the node
          ...
          }catch(RPG_ScenarioManagementException e){
  ...
  public static void main(String args[]) {
    ...
    try {
      ...
      java.util.logging.Logger.getLogger(IHM_ScenarioCreation.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
    ...
    java.awt.EventQueue.invokeLater(new Runnable() {
      public void run() {
        new IHM_ScenarioCreation().setVisible(true);
      }
    });
  }
