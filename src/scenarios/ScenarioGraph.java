...
import ihm.IHM_ScenarioCreation;
 ...
 * @author Nicolas Brax
 */
public class ScenarioGraph implements ViewerListener, MouseInputListener{
   ...
   */
  private IHM_ScenarioCreation ihm;
   ...
   */
  public ScenarioGraph(IHM_ScenarioCreation ihm){
    ...
    this.graph.addAttribute("ui.stylesheet", "url('file:" + System.getProperty("user.dir") 
            + "/data/SC_graphstream_ressources/graph_style.css'" + ")");
   ...
   */
  public void addLink(String fromId,String toId) throws RPG_ScenarioManagementException{
    if(!linkIds.contains(fromId+toId)){
      ...
      throw new RPG_ScenarioManagementException("The edge is already existing.");
    }
  }
...
//       else{
//         System.out.println("This node is not under the mouse." + x + "-" + y + "-" + e.getPoint().x + "-" + e.getPoint().y);
//       }
    }//for
    if(!"".equals(nodeId)){                                                     // if a node has been found
      ...
      if(e.getButton() == MouseEvent.BUTTON3){                                  // click on the right button
        ...
        if("".equals(firstSelectedForLinkCreation)){                            // there is no beginning for the edge yet
          ...
          }catch(RPG_ScenarioManagementException ex){
